...
namespace ThemeSwitcher {
    public class iRaspbian : Gtk.Box {
        public iRaspbian () {
            var wrapper = new Gtk.Box (Gtk.Orientation.VERTICAL, 0);
            wrapper.get_style_context ().add_class ("theme");

	        var pixbuf = new Gdk.Pixbuf.from_file("/usr/local/share/themeswitcher/iraspbian.png");
			...
			var button = new Gtk.Button.with_label ("iRaspbian...");
			button.get_style_context().add_class ("themeswitcher_button");
			button.clicked.connect (() => {
		        try {
		        	GLib.AppInfo info = AppInfo.create_from_commandline("xfce4-terminal --title=Config --hide-menubar --hide-borders --hide-scrollbar -e \"/usr/share/ThemeSwitcher/ThemeSwitcher.sh iRaspbian\"", null, AppInfoCreateFlags.SUPPORTS_STARTUP_NOTIFICATION);
