...
namespace ThemeSwitcher {
    public class TwisterOS : Gtk.Box {
        public TwisterOS () {
	        ...
	        var pixbuf = new Gdk.Pixbuf.from_file("/usr/local/share/themeswitcher/twisteros.png");
			...
			var button = new Gtk.Button.with_label ("TwisterOS...");
			button.get_style_context().add_class ("themeswitcher_button");
			button.clicked.connect (() => {
		        try {
		        	GLib.AppInfo info = AppInfo.create_from_commandline("xfce4-terminal --title=Config --hide-menubar --hide-borders --hide-scrollbar -e \"/usr/share/ThemeSwitcher/ThemeSwitcher.sh TwisterOS\"", null, AppInfoCreateFlags.SUPPORTS_STARTUP_NOTIFICATION);
