...
namespace ThemeSwitcher {
    public class RaspbianXP : Gtk.Box {
        public RaspbianXP () {
			...
			var button = new Gtk.Button.with_label ("RaspbianXP...");
			button.get_style_context().add_class ("themeswitcher_button");
			button.clicked.connect (() => {
		        try {
		        	GLib.AppInfo info = AppInfo.create_from_commandline("xfce4-terminal --title=Config --hide-menubar --hide-borders --hide-scrollbar -e \"/usr/share/ThemeSwitcher/ThemeSwitcher.sh RaspbianXP\"", null, AppInfoCreateFlags.SUPPORTS_STARTUP_NOTIFICATION);
